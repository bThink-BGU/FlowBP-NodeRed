...
module.exports = function () {
  ...
  class Field {
    ...
    constructor(label, params = {
      types: ['str', 'msg', 'jsonata'], defaultValue: undefined, options: undefined, required: true
    }) {
      ...
      if (this.types.includes('select') || this.types.includes('multiSelect')) {
        let typesCopy = this.types.filter(type => !['msg', 'jsonata'].includes(type));
      ...
      if ((this.types.includes('select') || this.types.includes('multiSelect')) && (!this.options || this.options.length === 0)) {
    ...
    getOnEditPrepare() {
      if (this.types[0] === 'select') {
        return `$("#node-input-${this.name}").val('select');`
      } else if (this.types[0] === 'multiSelect') {
      ...
      $("#node-input-${this.name}R").typedInput({
        default: '${this.types[0]}',
        typeField: $("#node-input-${this.name}RType"),
        types: [${this.types.map(t => `'${t}'`).join(',')}]
      });`
      }
    }
    ...
    getAttributes() {
      ...
      ${this.name}R: { value: "${this.defaultValue[0]}", required: true, validate: RED.validators.typedInput("${this.name}RType", false) },
