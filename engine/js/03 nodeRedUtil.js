...
RED.util = {
  ...
  evaluateEnvProperty: function (value, node) {
    ...
    if (/^\${[^}]+}$/.test(value)) {
      ...
      result = this.getSetting(node, name);
    } else if (!/\${\S+}/.test(value)) {
      // ENV_VAR
      result = this.getSetting(node, value);
    } else {
      // FOO${ENV_VAR}BAR
      return value.replace(/\${([^}]+)}/g, function (match, name) {
        var val = this.getSetting(node, name);
  ...
  evaluateNodeProperty: function (value, type, node, msg, callback) {
    ...
    if (type === 'str') {
      ...
      result = this.evaluateJSONataExpression(expr, msg);
    } else if (type === 'env') {
